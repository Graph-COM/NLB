...
def to_csv(data_name):
    a = open('./DATA/{}.txt'.format(data_name), "r")
    f = open('./DATA/{}.csv'.format(data_name), "w")
...
def run(args):
    ...
    if (data_name in ['UBUNTU', 'WIKITALK', 'sx-superuser']):
        to_csv(data_name)
    node_edge_feat_dim = args.node_edge_feat_dim
    PATH = './DATA/{}.csv'.format(data_name)
    OUT_DF = './DATA/{}/edges.csv'.format(data_name)
    OUT_FEAT = './DATA/{}/edge_feats.npy'.format(data_name)
    OUT_NODE_FEAT = './DATA/{}/edge_feats.npy'.format(data_name)
...
parser.add_argument('--dataset', choices = ['wikipedia', 'reddit', 'socialevolve', 'socialevolve_1month', 'socialevolve_2weeks', 'WIKITALK', 'sx-superuser', 'mooc', 'UBUNTU'], 
